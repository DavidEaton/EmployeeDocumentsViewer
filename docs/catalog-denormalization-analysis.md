...
The viewer currently reads from `Common.EmployeeDocumentCatalog` and left-joins to `HR.EmployeeDocumentsLookup` by `EmployeeId` to project employee-facing fields (`Employee`, `Department`, `Active`).

## Proposed change

Denormalize lookup fields into `Common.EmployeeDocumentCatalog` during indexing/backfill so the viewer can read from one table and avoid join work.

## Assessment

This is directionally strong **if the product tolerates eventual consistency** for employee attributes.
...
- Employee attributes (`NameLastFirst`, `HomeDepartment`, `Active`) change independently of documents.
  - If stored in catalog rows, values can become stale unless refresh logic exists.
...
1. Add denormalized columns to `Common.EmployeeDocumentCatalog`:
...
3. Add a periodic HR-delta refresh job in indexer(s) to update catalog rows for changed employees.
...
The proposal is a good optimization and architecture simplification for the viewer, with the key tradeoff that consistency moves from query-time to pipeline-time. If you can operationalize periodic metadata refresh and accept eventual consistency, this is likely a net win.
